...
class PW_Transients_Manager {
	...
	*/
	public function __construct() {
		...
		add_action( 'network_admin_menu', array( $this, 'tools_link' ) );
		...
		add_action ('delete_all_transients_hook',array($this,'delete_all_transients'),-1);
	}
	...
	*/
	public function tools_link() {
		...
		add_submenu_page('pw-transients-manager',
			__( 'Transients Cleanup Schedule', 'transients-manager' ),
			__( 'Scheduled Cleanups', 'transients-manager' ),
			'manage_options',
			'pw-transients-manager-scheduler',
			array( $this, 'scheduled_transients' )
		);
...
<?php
	...
	public function scheduled_transients(){
		?>
		<div class="wrap">

			

				<h2><?php _e( 'Scheduled Transient Cleanup', 'transients-manager' ); ?></h2>

				<form method="post">
					<table class="wp-list-table widefat fixed posts striped">
						<tbody>
						<tr>
							<?php if(wp_next_scheduled( 'delete_all_transients_hook' )) :
								$nextSchedule = wp_next_scheduled( 'delete_all_transients_hook' );
								?>
								<td> <?php echo date('d/m/Y H:i:s',$nextSchedule) ?>
								<td><input type="hidden" name="action" value="unschedule_transient_deletion" />
					<input type="hidden" name="transient" value="all" />
					<?php wp_nonce_field( 'transient_manager' ); ?>
					<input type="submit" class="button secondary" value="<?php _e( 'Unschedule Transient Deletion', 'transients-manager' ); ?>" /> 
				</td>
							<?php else:?>
								<td>
									<select name="transient_deletion_schedule_interval">
										<?php 
										$schedules = wp_get_schedules();
										foreach ($schedules as $recurrence => $schedule):
											$value = $recurrence;
								...
								<td>
									<input type="hidden" name="action" value="schedule_deleting_all_transients" />
					<input type="hidden" name="transient" value="all" />
					<?php wp_nonce_field( 'transient_manager' ); ?>
					<input type="submit" class="button secondary" value="<?php _e( 'Schedule Deletion of All Transients', 'transients-manager' ); ?>" />
	...
	public function process_actions() {
		...
		switch( $_REQUEST['action'] ) {
			...
			case 'delete_all_transients' :
				...
				wp_safe_redirect( admin_url( 'network/admin.php?page=pw-transients-manager' ) ); exit;
				break;

			case 'schedule_deleting_all_transients':
				$interval = $_REQUEST['transient_deletion_schedule_interval'];
				$this->schedule_deleting_all_transients($interval);
				wp_safe_redirect(admin_url('network/admin.php?page=pw-transients-manager-scheduler' )); exit;
				break;
			
			case 'unschedule_transient_deletion':
				$this->unschedule_transient_deletion();
				wp_safe_redirect(admin_url('network/admin.php?page=pw-transients-manager-scheduler' )); exit;
				break;
	...
	*/
	...
	/**
	 * Schedule Deleting all transients
	 *
	 * @access public
	 * @return bool
	 */
	public function schedule_deleting_all_transients($interval) {
		if ( ! wp_next_scheduled( 'delete_all_transients_hook' ) ){
			$result = wp_schedule_event(time(),$interval,'delete_all_transients_hook');
	...
	public function scheduler_error_admin_notice(){
		?>
			<div class="notice error scheduler_errpr_notice is-dismissible" >
        		<p><?php _e( 'An error occurred while scheduling transient deletion', 'transient-manager' ); ?></p>
   		 	</div>
		<?php
	}

	/**
	 * Unscheduling the Transient Deletion
	 * 
	 * @access public
	 * @return bool
	 */

	public function unschedule_transient_deletion(){
		$timestamp = wp_next_scheduled('delete_all_transients_hook');
		if(!$timestamp) return $timestamp;
		wp_unschedule_event($timestamp,'delete_all_transients_hook');
		return true;
	}
